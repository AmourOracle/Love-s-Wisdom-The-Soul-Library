...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof testData === 'undefined' || !testData || typeof testData !== 'object') {
        console.error("錯誤：找不到有效的 testData。"); displayInitializationError("無法載入測驗數據。"); return;
    }
    if (!Array.isArray(testData.questions) || testData.questions.length === 0) {
        console.error("錯誤：testData.questions 不是有效的陣列或為空。"); displayInitializationError("測驗問題數據格式錯誤。"); return;
    }
    ...
    function cacheDOMElements() {
         try {
            ...
            if (!DOM.containers.intro || !DOM.containers.test || !DOM.containers.result || !DOM.buttons.start || !DOM.elements.testBackground || !DOM.containers.options || !DOM.elements.questionTitle) {
                 console.error("錯誤：未能找到所有必要的 HTML 元素。"); displayInitializationError("頁面結構錯誤，無法啟動測驗。"); return false;
            }
    ...
    function switchScreen(fromScreenId, toScreenId) {
        ...
        setTimeout(() => {
            ...
            if (toScreenId === 'test') { initializeTestScreen(); state.contentRendered = true; } // 每次進入都初始化
    ...
    function updateProgressBar(questionNumber) {
        ...
        const currentQ = Math.max(0, Math.min(questionNumber -1, questions.length));
    ...
    function prepareNextQuestion() { /* ... (保持不變) ... */ }
    ...
    function bindStartButton() {
        if (DOM.buttons.start) {
            ...
            if (!DOM.buttons.start.dataset.listenerAttached) {
                ...
                DOM.buttons.start.dataset.listenerAttached = 'true'; // 添加標記
